...
class LiveSiteControlProvider extends ServiceProvider {
	...
	public function boot() {
		...
		$gdl_enabled        = 'gdl_enabled';
		$gdl_site_published = 'gdl_site_published';

		$gdl_enabled_option = $wpdb->get_var(
			$wpdb->prepare(
		...
		$gdl_site_published_option = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT option_value FROM {$wpdb->options} WHERE option_name = %s LIMIT 1",
				$gdl_site_published
			)
		...
		add_action(
			...
			function( $hook_suffix ) use ( $gdl_enabled_option ) {
		...
		if ( empty( $gdl_enabled_option ) && empty( $gdl_site_published_option ) ) {
